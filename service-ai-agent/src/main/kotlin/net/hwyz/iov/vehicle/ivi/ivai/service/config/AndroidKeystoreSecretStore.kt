...
/**
 * [SecretStore] backed by the Android Keystore (IVI-IVAI-DSN-CR-003 / IVAI-REQ-027):
 *  - non-exportable AES/GCM 256 master key generated in the AndroidKeyStore
 *  - each encryption uses a fresh random 12-byte IV; ciphertext+IV are Base64
 *    encoded into a private app file (never plaintext on disk)
 *  - corrupt / undecryptable ciphertext or an invalidated key → INVALID status;
 *    the raw key is never exposed in logs, exceptions or [toString]
 */
class AndroidKeystoreSecretStore(context: Context) : SecretStore {
    ...
    private companion object {
        ...
        const val KEY_ALIAS = "ivai_llm_api_key_master"
        const val CIPHER_FILE_NAME = "llm_api_key_v1.bin"
