...
object ToolRetrievalPayloadFactory {
    ...
    fun fromTool(
        ...
        val effectiveConflicts = if (conflictToolIds.isNotEmpty()) conflictToolIds else {
            boundary?.conflictToolIds ?: emptySet()
        }
        return ToolRetrievalSemanticPayload(
            ...
            positiveExamples = (boundary?.positiveExamples.orEmpty() + tool.positiveExamples).distinct(),
            negativeBoundaries = (boundary?.negativeExamples.orEmpty() + tool.negativeExamples).distinct(),
            conflictingToolIds = effectiveConflicts,
            sourceVersions = sourceVersions + setOfNotNull(boundary?.governanceVersion),
